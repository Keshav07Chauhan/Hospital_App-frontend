...
const BookNow = ({ date, id, email }) => {
    ...
    const getDoctorsDetails = async () => {
        try {
            const dataset = await axios.get(`https://hospital-app-backend-60sw.onrender.com/getdoctoravailability`, { params: { id: id } });
    ...
    const getAlreadyBookedData = async () => {
        try {
            const dataset = await axios.get(`https://hospital-app-backend-60sw.onrender.com/getalreadybookedslots`, { params: { id: id } });
            console.log(dataset);
    ...
    const bookSlot = async () => {
        ...
        try {
            ...
            await axios.post(`https://hospital-app-backend-60sw.onrender.com/submitbookings`, bookingData, { headers: { 'Authorization': `${token}` } });
            alert('slot booked');
        } catch (error) {
            alert(error);
        }
