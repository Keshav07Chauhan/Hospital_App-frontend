...
const DynamicGraph = () => {
    ...
    const doctorAndPatientCount = async () => {
        ...
        try {
            const response = await axios.get(`https://hospital-app-backend-60sw.onrender.com/getTotalPatientByDoctor`, { headers: { 'Authorization': `${token}` } });
    ...
    const dateAndPatientCount = async () => {
        const token = localStorage.getItem('authToken');
        try {
            const response = await axios.get(`https://hospital-app-backend-60sw.onrender.com/getTotalPAtientByDate`, { headers: { 'Authorization': `${token}` } });
